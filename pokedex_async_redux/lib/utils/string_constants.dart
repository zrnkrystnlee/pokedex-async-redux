const offsetPokemon = '0';
const limitPokemon = '151';

///Pokemon Overview Page String Constants
const imageUrl = 'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/home/[index].png';
const indexKey = '[index]';
const pokemonOverviewTitle = 'POKEDÉX';
const type = 'Type';
const typePrimary = 'Type 1';

///Pokemon Loading and Error Message
const pikachuRunningImage = 'assets/images/pikachu-running.gif';
const pokedexErrorImage = 'assets/images/pokedex-glitch.gif';
const errorMessage = 'Uh Oh! Pokedex not working!; Return to Professor Oak';
const noPokemonsAvailableLabel = 'No Pokemons Available!';
const emptyString = '';

///Pokemon Details Constants
const heightLabel = 'Height';
const cmLabel = 'cm';
const weightLabel = 'Weight';
const kgLabel = 'kg';
const statsLabel = 'Stats';
const baseExpLabel = 'Base Exp';
const abilityLabel = 'Ability';
const movesLabel = 'Moves';
const hpLabel = 'HP';
const attackLabel = 'Attack';
const defenseLabel = 'Defense';
const spAttackLabel = 'Sp.Attack';
const spDefenseLabel = 'Sp.Defense';
const speedLabel = 'Speed';
const unknownLabel = 'Unknown';
const imageName = 'imageName';
const title = 'title';
const hpCase = 'hp';
const attackCase = 'attack';
const defenseCase = 'defense';
const spAttackCase = 'special-attack';
const spDefCase = 'special-defense';
const speedCase = 'speed';
const noDetailsAvailableLabel = 'The Details are not Avaialble!';

///Pokemon Details Images Constants
const pokeballLoading = 'assets/images/pokeball-spinning.gif';
const pokeballTransparent = 'assets/images/pokeball-clear.png';
const clearPokeball = 'assets/images/pokeball-clear.png';
const heartImage = 'assets/images/heart-image.png';
const swordImage = 'assets/images/sword-image.png';
const shieldImage = 'assets/images/sheild-image.png';
const spAttackImage = 'assets/images/spAttack-image.png';
const spShieldImage = 'assets/images/spSheild-image.png';
const speedImage = 'assets/images/speed-image.png';
const pokeballImage = 'assets/images/pokeball-image.png';
